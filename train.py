...
def train_rnn_epoch(epoch, args, rnn, output, data_loader,
    ...
    for batch_idx, data in enumerate(data_loader):
        ...
        # if using ground truth to train
        h = rnn(x, cond=Variable(torch.FloatTensor(y_len)).cuda(), pack=True, input_len=y_len)  # <----- condition created here
...
def test_rnn_epoch(epoch, args, rnn, output, test_batch_size=16):
    ...
    x_step = Variable(torch.ones(test_batch_size,1,args.max_prev_node)).cuda()
    for i in range(max_num_node):
        cond_var = (torch.ones(test_batch_size) * 23).cuda()
        h = rnn(x_step, cond=cond_var)
    ...
    while epoch<=args.epochs:
        ...
        if 'GraphRNN_VAE' in args.note:
            ...
            train_mlp_epoch(epoch, args, rnn, output, dataset_train,
                            optimizer_rnn, optimizer_output,
                            scheduler_rnn, scheduler_output)
        elif 'GraphRNN_RNN' in args.note or 'GraphRNN_cond' in args.note:
        ...
        # test
        if True: #epoch % args.epochs_test == 0 and epoch>=args.epochs_test_start:
